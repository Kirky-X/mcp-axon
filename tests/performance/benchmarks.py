...
def benchmark_crud_operations():
    ...
    print(f"创建 1000 条: {create_time*1000:.2f}ms (平均 {create_time*1000/1000:.2f}ms/条)")
    print(f"查询所有记录: {query_time*1000:.2f}ms")
    print(f"更新 100 条: {update_time*1000:.2f}ms (平均 {update_time*1000/100:.2f}ms/条)")
    print(f"删除 100 条: {delete_time*1000:.2f}ms (平均 {delete_time*1000/100:.2f}ms/条)")
...
def main():
    ...
    # 性能要求检查
    print(f"\n=== 性能要求检查 ===")
    print(f"CRUD < 50ms: {'✅ 通过' if crud_results['create_avg_ms'] < 50 else '❌ 失败'} ({crud_results['create_avg_ms']:.2f}ms)")
    print(f"拓扑排序 < 1000ms: {'✅ 通过' if sort_results[2000] < 1000 else '❌ 失败'} ({sort_results[2000]:.2f}ms)")
    print(f"链化 < 2000ms: {'✅ 通过' if chain_results[1000]['chain_time_ms'] < 2000 else '❌ 失败'} ({chain_results[1000]['chain_time_ms']:.2f}ms)")
