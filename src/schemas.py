...
class RequirementCreate(BaseModel):
    ...
    parent_id: Optional[str] = Field(None, min_length=36, max_length=36, description="父需求 ID")
...
class ValidationCreate(BaseModel):
    """验证节点创建 Schema"""
    requirement_id: str = Field(..., min_length=36, max_length=36, description="需求 ID")
...
class ParallelOrderResolve(BaseModel):
    ...
    parallel_nodes: List[str] = Field(
        ...,
        min_length=1,
        description="并行节点 ID 列表"
    )
    sorted_order: List[str] = Field(
        ...,
        min_length=1,
        description="排序后的节点 ID 列表"
    )
