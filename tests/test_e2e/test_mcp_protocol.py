...
async def test_mcp_tool_call_mark_as_leaf():
    ...
    # Act
    result = await execute_tool("mark_as_leaf", {"requirement_id": req_id})
...
async def test_mcp_complete_workflow():
    ...
    project = await execute_tool(
        "create_project", {"name": "完整测试项目", "description": "测试完整流程"}
    )
    project_id = project["project_id"]

    # Step 2: 添加多个需求
    req1 = await execute_tool(
        "add_requirement", {"project_id": project_id, "content": "需求1"}
    )
    req2 = await execute_tool(
        "add_requirement", {"project_id": project_id, "content": "需求2"}
    )
    req3 = await execute_tool(
        "add_requirement", {"project_id": project_id, "content": "需求3"}
    )
    ...
    await execute_tool(
        "add_validation",
        {"requirement_id": req1["requirement_id"], "test_cases": [{"name": "测试1"}]},
    )
    await execute_tool(
        "add_validation",
        {"requirement_id": req2["requirement_id"], "test_cases": [{"name": "测试2"}]},
    )
    await execute_tool(
        "add_validation",
        {"requirement_id": req3["requirement_id"], "test_cases": [{"name": "测试3"}]},
    )
    ...
    await execute_tool(
        ...
        {"project_id": project_id, "requirement_id": next_req["requirement_id"]},
    )
...
async def test_mcp_error_handling():
    ...
    await execute_tool("create_project", {"name": "测试项目"})
    with pytest.raises(Exception):
        await execute_tool(
            "add_requirement", {"project_id": "nonexistent-id", "content": "测试"}
        )

    # Test 2: 获取不存在的项目 - 应该抛出异常
    with pytest.raises(Exception):
        await execute_tool("get_project", {"project_id": "invalid-id-12345"})

    # Test 3: 删除不存在的需求 - 应该抛出异常
    with pytest.raises(Exception):
        await execute_tool("delete_requirement", {"requirement_id": "nonexistent-id"})
...
async def test_mcp_lock_mechanism():
    """测试锁机制"""
    # 创建项目
    project = await execute_tool("create_project", {"name": "锁测试项目"})
    project_id = project["project_id"]

    # 获取锁
    lock_result = await execute_tool(
        "acquire_lock", {"project_id": project_id, "session_id": "session-1"}
    )
    ...
    lock_result3 = await execute_tool(
        "acquire_lock", {"project_id": project_id, "session_id": "session-2"}
    )
    assert lock_result3["success"] is False

    # 释放锁
    release_result = await execute_tool(
        "release_lock", {"project_id": project_id, "session_id": "session-1"}
    )
...
async def test_mcp_dependency_management():
    """测试依赖关系管理"""
    # 创建项目
    project = await execute_tool("create_project", {"name": "依赖测试项目"})
    project_id = project["project_id"]

    # 创建有依赖关系的需求
    req_a = await execute_tool(
        "add_requirement", {"project_id": project_id, "content": "需求A"}
    )
    req_b = await execute_tool(
        "add_requirement", {"project_id": project_id, "content": "需求B"}
    )
    req_c = await execute_tool(
        "add_requirement", {"project_id": project_id, "content": "需求C"}
    )
    ...
    await execute_tool(
        "add_validation",
        {"requirement_id": req_a["requirement_id"], "test_cases": [{"name": "测试A"}]},
    )
    await execute_tool(
        "add_validation",
        {"requirement_id": req_b["requirement_id"], "test_cases": [{"name": "测试B"}]},
    )
    await execute_tool(
        "add_validation",
        {"requirement_id": req_c["requirement_id"], "test_cases": [{"name": "测试C"}]},
    )
...
async def test_mcp_update_operations():
    ...
    # 创建项目
    project = await execute_tool(
        "create_project", {"name": "原始名称", "description": "原始描述"}
    )
    project_id = project["project_id"]
    ...
    req = await execute_tool(
        "add_requirement", {"project_id": project_id, "content": "原始需求内容"}
    )

    # 更新需求内容
    updated_req = await execute_tool(
        "update_requirement",
        {"requirement_id": req["requirement_id"], "content": "更新后的需求内容"},
    )
...
async def test_mcp_snapshot_management():
    """测试快照管理"""
    # 创建项目和需求
    project = await execute_tool("create_project", {"name": "快照测试项目"})
    project_id = project["project_id"]
    ...
    # 创建快照
    snapshot_result = await execute_tool("create_snapshot", {"project_id": project_id})
    snapshot_id = snapshot_result["snapshot_id"]
    assert snapshot_id is not None

    # 列出快照
    snapshots = await execute_tool(
        "list_snapshots", {"project_id": project_id, "limit": 10}
    )
    ...
    await execute_tool(
        "update_requirement",
        {"requirement_id": req["requirement_id"], "content": "修改后的内容"},
    )
...
async def test_mcp_delete_operations():
    """测试删除操作"""
    # 创建项目
    project = await execute_tool("create_project", {"name": "删除测试项目"})
    project_id = project["project_id"]

    # 添加需求
    req = await execute_tool(
        "add_requirement", {"project_id": project_id, "content": "待删除的需求"}
    )
    req_id = req["requirement_id"]

    # 删除需求
    delete_result = await execute_tool("delete_requirement", {"requirement_id": req_id})
...
async def test_mcp_transfer_dependencies():
    """测试依赖传递"""
    # 创建项目
    project = await execute_tool("create_project", {"name": "依赖传递测试项目"})
    project_id = project["project_id"]

    # 创建父需求
    parent = await execute_tool(
        "add_requirement", {"project_id": project_id, "content": "父需求"}
    )
    ...
    dep1 = await execute_tool(
        "add_requirement", {"project_id": project_id, "content": "依赖需求1"}
    )
    dep2 = await execute_tool(
        "add_requirement", {"project_id": project_id, "content": "依赖需求2"}
    )
    ...
    await execute_tool(
        "add_validation",
        {"requirement_id": dep1["requirement_id"], "test_cases": [{"name": "测试"}]},
    )
    await execute_tool(
        "add_validation",
        {"requirement_id": dep2["requirement_id"], "test_cases": [{"name": "测试"}]},
    )
...
async def test_mcp_resolve_parallel_order():
    """测试并行节点排序"""
    # 创建项目
    project = await execute_tool("create_project", {"name": "并行排序测试项目"})
    project_id = project["project_id"]

    # 创建多个独立需求（并行节点）
    req1 = await execute_tool(
        "add_requirement", {"project_id": project_id, "content": "并行需求1"}
    )
    req2 = await execute_tool(
        "add_requirement", {"project_id": project_id, "content": "并行需求2"}
    )
    req3 = await execute_tool(
        "add_requirement", {"project_id": project_id, "content": "并行需求3"}
    )
    ...
    await execute_tool(
        "add_validation",
        {"requirement_id": req1["requirement_id"], "test_cases": [{"name": "测试"}]},
    )
    await execute_tool(
        "add_validation",
        {"requirement_id": req2["requirement_id"], "test_cases": [{"name": "测试"}]},
    )
    await execute_tool(
        "add_validation",
        {"requirement_id": req3["requirement_id"], "test_cases": [{"name": "测试"}]},
    )
...
async def test_mcp_large_scale_project():
    """测试大规模项目"""
    # 创建项目
    project = await execute_tool("create_project", {"name": "大规模测试项目"})
    project_id = project["project_id"]

    # 创建根需求
    root = await execute_tool(
        "add_requirement", {"project_id": project_id, "content": "根需求"}
    )
    ...
    for child_id in child_ids:
        await execute_tool(
            "add_validation",
            {"requirement_id": child_id, "test_cases": [{"name": f"测试{i}"}]},
        )
...
async def test_mcp_concurrent_lock_scenarios():
    """测试并发锁场景"""
    # 创建项目
    project = await execute_tool("create_project", {"name": "并发锁测试项目"})
    project_id = project["project_id"]
    ...
    release2 = await execute_tool(
        "release_lock", {"project_id": project_id, "session_id": "session-2"}
    )
...
async def test_mcp_nested_requirements():
    """测试嵌套需求结构"""
    # 创建项目
    project = await execute_tool("create_project", {"name": "嵌套需求测试项目"})
    project_id = project["project_id"]

    # 创建多层嵌套需求
    level0 = await execute_tool(
        "add_requirement", {"project_id": project_id, "content": "第0层"}
    )
    ...
    await execute_tool(
        "add_validation",
        {"requirement_id": level3["requirement_id"], "test_cases": [{"name": "测试"}]},
    )
...
async def test_mcp_circular_dependency_prevention():
    """测试循环依赖预防"""
    # 创建项目
    project = await execute_tool("create_project", {"name": "循环依赖测试项目"})
    project_id = project["project_id"]
    ...
    await execute_tool(
        "add_validation",
        {"requirement_id": req_a["requirement_id"], "test_cases": [{"name": "测试"}]},
    )
    await execute_tool(
        "add_validation",
        {"requirement_id": req_b["requirement_id"], "test_cases": [{"name": "测试"}]},
    )
    await execute_tool(
        "add_validation",
        {"requirement_id": req_c["requirement_id"], "test_cases": [{"name": "测试"}]},
    )
