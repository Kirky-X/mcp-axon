...
async def execute_tool(name: str, arguments: dict) -> dict:
    ...
    if name == "create_project":
        ...
        return sdk.get_next_requirement(project_id=arguments["project_id"])
        ...
        return sdk.trigger_chaining(project_id=arguments["project_id"])

    elif name == "create_snapshot":
        snapshot_id = sdk.create_snapshot(project_id=arguments["project_id"])
        return {
        ...
        return sdk.restore_snapshot(snapshot_id=arguments["snapshot_id"])
        ...
        return {
            "locked": locked,
            "message": "项目已锁定" if locked else "项目未锁定"
        }
...
def validate_tool_input(name: str, arguments: dict) -> None:
    ...
    # 项目 ID 验证
    if name in ["update_project", "get_project", "add_requirement", "get_project_state", 
                "trigger_chaining", "create_snapshot", "list_snapshots", "acquire_lock", 
                "release_lock", "is_locked", "get_lock_info", "resolve_parallel_order", 
                "get_next_requirement", "mark_requirement_completed"]:
        project_id = arguments.get("project_id")
        if not project_id:
            raise ValueError("project_id 参数是必填的")
        if not uuid_pattern.match(project_id):
            raise ValueError("project_id 格式不正确，必须是有效的 UUID")
    
    # 需求 ID 验证
    if name in ["update_requirement", "mark_as_leaf", "delete_requirement", "add_validation",
                "add_dependency"]:
        requirement_id = arguments.get("requirement_id")
        if not requirement_id:
            raise ValueError("requirement_id 参数是必填的")
    ...
    if name == "transfer_dependencies":
        parent_id = arguments.get("parent_id")
        if not parent_id:
            raise ValueError("parent_id 参数是必填的")
